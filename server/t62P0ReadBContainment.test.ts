...
describe("T62 P0-read B map credential and webhook response containment", () => {
  ...
  it("uses the published Building Footprints identifier without broadening its field request", () => {
    ...
    expect(mapPageSource).toContain('outFields: "building_id,OBJECTID"');
    expect(mapPageSource).toContain("feature.attributes?.building_id");
    expect(mapPageSource).not.toContain('outFields: "*"');
  });
