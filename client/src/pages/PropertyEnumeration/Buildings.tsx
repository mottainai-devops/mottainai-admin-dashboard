...
import { Building2, MapPin, User, Users, Calendar, Download, Search, Filter, Pencil, Check, X, Package } from "lucide-react";
...
function ViewCustomersButton({ arcgisBuildingId }: { arcgisBuildingId: string }) {
  const [, setLocation] = useLocation();
  return (
    <Button
      variant="outline"
      className="w-full"
      onClick={() => setLocation(`/customers?arcgisBuildingId=${encodeURIComponent(arcgisBuildingId)}`)}
    >
      <Users className="mr-2 h-4 w-4" />
      View Customers for this Building
    </Button>
  );
}
            ...
            <div className="space-y-6">
              ...
              {buildingDetails.arcgisBuildingId && (
                <div className="pt-2 border-t space-y-2">
                  ...
                  <ViewCustomersButton arcgisBuildingId={buildingDetails.arcgisBuildingId} />
                </div>
              )}
            </div>
          ) : null}
